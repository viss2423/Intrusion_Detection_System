...
class MaliciousTrafficGenerator:
    ...
    def send_malicious_traffic(self, packet_count=500):
        for _ in range(packet_count):
            # Craft an ICMP Echo Request packet with random data
            payload = bytes([random.randint(0, 255) for _ in range(32)])  # Replace 32 with your desired payload length
...
# Function to perform intrusion detection for a given packet
def perform_intrusion_detection(packet, model, scaler, label_encoder):
    ...
    if len(features) > 0:
        ...
        if feature_vector is not None:
            ...
            # Return whether the packet is benign or malignant
            if decoded_prediction[0] == "BENIGN":
                return "BENIGN"
            else:
                return "MALIGNANT"

    # Return None if no prediction is made
    return None
...
if __name__ == "__main":
    ...
    # Send malicious traffic and perform intrusion detection on each packet
    for _ in range(500):
        # Generate and send malicious traffic
        malicious_traffic_generator.send_malicious_traffic()

        # Define a capture object for packet processing
        capture = pyshark.LiveCapture(interface="Wi-Fi", tshark_path=tshark_path, display_filter="")

        for packet in capture.sniff_continuously():
            result = perform_intrusion_detection(packet, model, scaler, label_encoder)
            
            if result:
                print(f"Packet Prediction: {result}")
